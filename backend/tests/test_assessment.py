...
# Stichtag ohne laufende Grosslage. Ohne ihn haengen die Ergebnisse davon ab,
# an welchem Kalendertag die Tests laufen — waehrend Puetzchens Markt laeuft,
# kommt ein zusaetzliches Signal hinzu.
...
def test_gleiche_warnung_im_eigenen_kreis_wiegt_deutlich_schwerer():
    ...
    fern = assess_deployment(_scores(
        official_warning={"score": 90, "area_scope": "ausserhalb"}), LABELS, tag=RUHIGER_TAG)
    nah = assess_deployment(_scores(
        official_warning={"score": 90, "area_scope": "troisdorf"}), LABELS, tag=RUHIGER_TAG)
...
def test_warnung_ohne_ortsangabe_wird_konservativ_eingeordnet():
    ...
    ohne = assess_deployment(_scores(official_warning={"score": 80}), LABELS, tag=RUHIGER_TAG)
    nah = assess_deployment(_scores(
        official_warning={"score": 80, "area_scope": "troisdorf"}), LABELS, tag=RUHIGER_TAG)
    fern = assess_deployment(_scores(
        official_warning={"score": 80, "area_scope": "ausserhalb"}), LABELS, tag=RUHIGER_TAG)
...
def test_hochwasser_schlaegt_direkt_auf_die_bereitschaft_durch():
    ...
    result = assess_deployment(_scores(water=85), LABELS, tag=RUHIGER_TAG)
...
def test_manv_wiegt_weniger_als_eine_betreuungslage():
    ...
    """
    manv = assess_deployment(_scores(manv=80), LABELS, tag=RUHIGER_TAG)
    wasser = assess_deployment(_scores(water=80), LABELS, tag=RUHIGER_TAG)
    assert manv["value"] < wasser["value"]


def test_gesundheit_wiegt_wenig_mangels_rettungsdienst():
    gesundheit = assess_deployment(_scores(health=90), LABELS, tag=RUHIGER_TAG)
...
def test_erdbeben_gleicher_hoehe_bleibt_deutlich_darunter():
    ...
    wasser = assess_deployment(_scores(water=85), LABELS, tag=RUHIGER_TAG)
    beben = assess_deployment(_scores(seismic=85), LABELS, tag=RUHIGER_TAG)
...
def test_nachrichten_sind_der_staerkste_fruehindikator():
    ...
    ortsnah = assess_deployment(_scores(
        news={"score": 85, "detail": "Grossbrand in Troisdorf"}), LABELS, tag=RUHIGER_TAG)
...
def test_nachricht_ohne_ortsbezug_wiegt_weniger():
    ...
    fern = assess_deployment(_scores(news={"score": 85}), LABELS, tag=RUHIGER_TAG)
    nah = assess_deployment(_scores(
        news={"score": 85, "detail": "Lage in Siegburg"}), LABELS, tag=RUHIGER_TAG)
...
def test_strahlung_wird_nie_abgewertet():
    ...
    result = assess_deployment(_scores(radiation=85), LABELS, tag=RUHIGER_TAG)
    assert _level(result) == "einsatz_wahrscheinlich"
...
def test_stromausfall_ausserhalb_wiegt_weniger_als_vor_ort():
    ...
    ort = assess_deployment(_scores(
        power={"score": 80, "primary_condition": "outage"}), LABELS, tag=RUHIGER_TAG)
    fern = assess_deployment(_scores(
        power={"score": 80, "primary_condition": "extremlage_ausserhalb"}), LABELS, tag=RUHIGER_TAG)
...
def test_grosslage_im_kreis_liegt_zwischen_ort_und_ausserhalb():
    ...
    kreis = assess_deployment(_scores(
        power={"score": 80, "primary_condition": "grosslage_kreis"}), LABELS, tag=RUHIGER_TAG)
    fern = assess_deployment(_scores(
        power={"score": 80, "primary_condition": "extremlage_ausserhalb"}), LABELS, tag=RUHIGER_TAG)
...
def test_mehrere_erhoehte_kategorien_verschaerfen_die_erwartung():
    einzeln = assess_deployment(_scores(weather=70), LABELS, tag=RUHIGER_TAG)
    mehrfach = assess_deployment(_scores(weather=70, water=60, power=60), LABELS, tag=RUHIGER_TAG)
...
def test_zuschlag_hebt_hoechstens_eine_stufe():
    ...
    viele = assess_deployment(_scores(
        weather=50, water=50, power=50, fire=50, traffic=50, health=50), LABELS, tag=RUHIGER_TAG)
    eine_schwere = assess_deployment(_scores(water=95), LABELS, tag=RUHIGER_TAG)
    assert eine_schwere["value"] > viele["value"]


def test_nebenlagen_unter_der_schwelle_zaehlen_nicht():
    unter = NEBENLAGE_SCHWELLE - 10
    result = assess_deployment(_scores(water=80, weather=unter / 0.9 - 1), LABELS, tag=RUHIGER_TAG)
...
def test_ruhige_lage_ergibt_ruhe():
    result = assess_deployment(_scores(), LABELS, tag=RUHIGER_TAG)
    assert _level(result) == "ruhe"
    assert result["value"] == 0.0


def test_leere_eingabe_bricht_nicht():
    result = assess_deployment({}, LABELS, tag=RUHIGER_TAG)
    assert _level(result) == "ruhe"
...
def test_begruendung_nennt_den_treibenden_anlass_mit_klarnamen():
    result = assess_deployment(_scores(water=80), LABELS, tag=RUHIGER_TAG)
    assert any("Hochwasser" in r for r in result["reasons"])


def test_stufen_steigen_monoton_mit_dem_score():
    werte = [assess_deployment(_scores(water=s), LABELS, tag=RUHIGER_TAG)["value"]
...
def test_komponenten_sind_dubletten_frei():
    ...
    result = assess_deployment(_scores(water=80, power=60), LABELS, tag=RUHIGER_TAG)
...
def test_vorlaufzeiten_stammen_aus_dem_bedarfsplan():
    ...
    result = assess_deployment(_scores(weather=70), LABELS, tag=RUHIGER_TAG)
...
def test_evakuierung_senkt_eine_hohe_bewertung_nicht():
    ...
    ohne = assess_deployment(_scores(water=95), LABELS, tag=RUHIGER_TAG)
    mit = assess_deployment(_scores(
        water={"score": 95, "detail": "Evakuierung in Troisdorf"}), LABELS, tag=RUHIGER_TAG)
    assert mit["value"] >= ohne["value"]


def test_evakuierungsbegruendung_steht_an_erster_stelle():
    result = assess_deployment(_scores(
        news={"score": 40, "detail": "Evakuierung in Troisdorf"}), LABELS, tag=RUHIGER_TAG)
...
def test_brandlage_bringt_die_kueche_ins_spiel():
    ...
    result = assess_deployment(_scores(fire=85), LABELS, tag=RUHIGER_TAG)
...
def test_veranstaltungen_bringen_den_sanitaetsdienst():
    result = assess_deployment(_scores(events=80), LABELS, tag=RUHIGER_TAG)
...
def test_begruendung_erklaert_das_standortprofil():
    ...
    result = assess_deployment(_scores(manv=80), LABELS, tag=RUHIGER_TAG)
...
def test_siegburg_zaehlt_wie_das_eigene_stadtgebiet():
    ...
    troisdorf = assess_deployment(_scores(
        news={"score": 80, "detail": "Grossbrand in Troisdorf"}), LABELS, tag=RUHIGER_TAG)
    siegburg = assess_deployment(_scores(
        news={"score": 80, "detail": "Grossbrand in Siegburg"}), LABELS, tag=RUHIGER_TAG)
...
def test_nachbargemeinden_liegen_knapp_unter_dem_kerngebiet():
    ...
    kern = assess_deployment(_scores(
        news={"score": 80, "detail": "Lage in Siegburg"}), LABELS, tag=RUHIGER_TAG)
    nachbar = assess_deployment(_scores(
        news={"score": 80, "detail": "Lage in Lohmar"}), LABELS, tag=RUHIGER_TAG)
    kreis = assess_deployment(_scores(
        news={"score": 80, "detail": "Lage in Windeck"}), LABELS, tag=RUHIGER_TAG)
...
def test_sankt_augustin_wird_in_beiden_schreibweisen_erkannt():
    ...
    lang = assess_deployment(_scores(
        news={"score": 80, "detail": "Lage in Sankt Augustin"}), LABELS, tag=RUHIGER_TAG)
    kurz = assess_deployment(_scores(
        news={"score": 80, "detail": "Lage in St. Augustin"}), LABELS, tag=RUHIGER_TAG)
...
def test_kerngebiet_hat_vorrang_vor_der_nachbarschaft():
    ...
    result = assess_deployment(_scores(
        news={"score": 40,
              "detail": "Evakuierung in Siegburg und Lohmar"}), LABELS, tag=RUHIGER_TAG)
...
def test_evakuierung_in_der_nachbarschaft_wiegt_weniger():
    ...
    kern = assess_deployment(_scores(
        news={"score": 20, "detail": "Evakuierung in Troisdorf"}), LABELS, tag=RUHIGER_TAG)
    nachbar = assess_deployment(_scores(
        news={"score": 20, "detail": "Evakuierung in Niederkassel"}), LABELS, tag=RUHIGER_TAG)
...
def test_ueberoertliche_lage_nennt_die_grundrate():
    ...
    """
    result = assess_deployment(_scores(
        official_warning={"score": 95, "area_scope": "ausserhalb"}), LABELS, tag=RUHIGER_TAG)
...
def test_ortsnahe_lage_nennt_die_grundrate_nicht():
    ...
    result = assess_deployment(_scores(
        water={"score": 85, "detail": "Pegel Troisdorf steigt"}), LABELS, tag=RUHIGER_TAG)
